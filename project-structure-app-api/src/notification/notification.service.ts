...
export class NotificationService {
    ...
    async subscribeToTopic(deviceTokens: string[], topic: string) {
        try {
            ...
            logData('Notification subscribed successfully', res);
            return { success: true, count: res.successCount };
        } catch (e: unknown) {
            logError('Notification subscription failed', e);
            return {
                success: false,
    ...
    async unsubscribeFromTopic(deviceTokens: string[], topic: string) {
        try {
            ...
            logData('Notification unsubscribed successfully', res);
            return { success: true, count: res.successCount };
        } catch (e: unknown) {
            logError('Notification unsubscription failed', e);
            return {
                success: false,
                error: e instanceof Error ? e.message : String(e),
            };
    ...
    async singleNotificationSend(
        ...
        if (details !== undefined) {
            messageBody.details = JSON.stringify(details);
        }
        if (sound_name) {
            messageBody.sound =
        ...
        try {
            ...
            logError('ERROR_SENDING_NOTIFICATION', error);
        }
    }
